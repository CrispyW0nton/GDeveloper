/**
 * ChatWorkspace — Sprint 12 + Sprint 16
 ...
 * Sprint 16 additions: unified ToolCallCard rendering, TaskPlanCard,
 * model picker in header, sandbox monitor integration.
 */
    ...
    const unsubscribe = api.onStreamChunk((data: any) => {
      ...
      if (data.type === 'text') {
        ...
        setStreamingToolCalls(prev => [
          ...prev,
          { name: data.toolCall.name, description: `Calling ${data.toolCall.name}...`, status: 'running' as const, input: data.toolCall.input },
        ]);
      } else if (data.type === 'tool_result' || data.type === 'tool_error') {
        setStreamingToolCalls(prev =>
          prev.map(tc =>
            tc.name === data.toolName
              ? { ...tc, status: data.type === 'tool_error' ? 'error' as const : 'success' as const, description: `${data.toolName}: ${(data.result || '').substring(0, 100)}`, result: data.result }
              : tc
          )
        );
