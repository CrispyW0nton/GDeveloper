...
function renderToolSummary(name: string, input: any, result: any): string {
  switch (name) {
    case 'multi_edit':
      return `${result?.file_path || input?.file_path || '?'} \u2014 ${result?.applied || '?'} edits`;
    case 'bash_command':
      return `$ ${(input?.command || '').substring(0, 80)} \u2192 exit ${result?.exit_code ?? '?'}`;
    case 'parallel_search':
      return `${result?.completed || '?'}/${result?.total_queries || '?'} searches completed`;
    case 'parallel_read':
      return `${result?.completed || '?'}/${result?.total_urls || '?'} URLs read`;
    case 'summarize_large_document':
      return `${result?.word_count?.toLocaleString() || '?'} words \u2014 ${(input?.question || '').substring(0, 60)}`;
    case 'task_plan':
      return `${result?.plan?.tasks?.length || '?'} tasks \u2014 ${result?.action || ''}`;
    case 'read_file':
      return input?.path || '';
    case 'write_file':
      return input?.path || '';
    case 'patch_file':
      return input?.path || '';
    default:
      return typeof result === 'string' ? result.substring(0, 100) : JSON.stringify(result || input).substring(0, 100);
  }
}
