...
const PERMISSIONS = [
  { name: 'Contents', access: 'Read & Write', icon: 'repo' },
  { name: 'Pull Requests', access: 'Read & Write', icon: 'pr' },
  { name: 'Issues', access: 'Read & Write', icon: 'issue' },
  { name: 'Metadata', access: 'Read', icon: 'meta' },
];
...
export default function GitHubPanel({ connected, selectedRepo, onConnect, onSelectRepo, onReposLoaded }: GitHubPanelProps) {
  ...
  const handleConnect = async () => {
    ...
    try {
      if (api) {
        ...
        if (result.success) {
          onConnect(result.username);
          setToken(''); // Clear token from UI after successful save
          // Repos will load via useEffect
        } else {
  ...
  const handleSelectRepo = async (repo: RepoInfo) => {
    if (api) {
      await api.selectRepo(repo.fullName);
    }
    onSelectRepo(repo);
  };
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-matrix-text-muted/50 mt-1">Connect with a Personal Access Token to browse and select repositories</p>
          </div>
        ...
        {error && (
          <div className="text-xs text-matrix-danger bg-matrix-danger/5 border border-matrix-danger/20 rounded px-4 py-3">
            {error}
          </div>
        )}
        ...
        {!connected && (
          <div className="glass-panel p-6">
            <div className="text-center mb-4">
              ...
              <h2 className="text-sm text-matrix-green mb-2 mt-3">Connect GitHub Account</h2>
              <p className="text-xs text-matrix-text-muted/40 mb-4">
                Enter a GitHub Personal Access Token (PAT) with repo access.<br/>
                <a href="https://github.com/settings/tokens/new?scopes=repo" target="_blank" rel="noreferrer" className="text-matrix-info/50 hover:text-matrix-info underline">
            ...
            <div className="mt-6 text-left max-w-md mx-auto">
              <p className="text-[10px] text-matrix-text-muted/40 mb-2 uppercase tracking-wider">Required Scopes</p>
              <div className="space-y-1.5">
                {PERMISSIONS.map(p => (
                  <div key={p.name} className="flex items-center justify-between text-[10px] px-3 py-1 bg-matrix-bg-hover/30 rounded">
                    <span className="text-matrix-text-dim">{p.name}</span>
        ...
        {connected && (
          <>
            <div className="flex gap-3">
              ...
              />
              <button className="matrix-btn" onClick={loadRepos} disabled={loading}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 11-2.12-9.36L23 10"/></svg>
                {loading ? 'Loading...' : 'Refresh'}
              </button>
              <button className="matrix-btn matrix-btn-danger text-xs" onClick={handleDisconnect}>
                Disconnect
              </button>
            </div>
            ...
            {loading && repos.length === 0 ? (
              <div className="glass-panel p-8 text-center">
                ...
                <p className="text-xs text-matrix-text-muted/50 mt-2">Loading repositories from GitHub...</p>
              </div>
            ) : repos.length === 0 ? (
              <div className="glass-panel p-8 text-center">
                <p className="text-xs text-matrix-text-muted/50">No repositories found. Make sure your token has repo access.</p>
              </div>
            ) : (
              <div className="space-y-2">
                <div className="text-[10px] text-matrix-text-muted/30 px-1">{filtered.length} repositories</div>
                {filtered.map(repo => (
                  <button
                    key={repo.id}
                    onClick={() => handleSelectRepo(repo)}
                    className={`w-full glass-panel p-4 text-left transition-all hover:border-matrix-green/50 ${
