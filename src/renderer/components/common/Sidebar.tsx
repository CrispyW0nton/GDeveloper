...
import { TabId, SelectedRepo } from '../../store';
...
export default function Sidebar({
  activeTab, onTabChange, repoSelected, githubConnected, apiKeyConfigured, selectedRepo, collapsed, onToggleCollapse
}: SidebarProps) {
  return (
    <aside className={`glass-panel-solid h-full flex flex-col transition-all duration-300 relative overflow-hidden ${collapsed ? 'w-14' : 'w-56'}`}>
      ...
      <nav className="relative z-10 flex-1 overflow-y-auto py-2">
        {NAV_ITEMS.map(item => {
          const disabled = item.requiresRepo && !repoSelected;
