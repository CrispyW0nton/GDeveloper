...
const electronAPI = {
  ...
  inspectIntent: (sessionId: string, message: string, options?: any) => ipcRenderer.invoke('intent:inspect', sessionId, message, options),
  ...
  // Sprint 28: Auto-continue preload bindings removed — loop is driven by stop_reason
  ...
  setOrchestrationSettings: (settings: any) => ipcRenderer.invoke('settings:set-orchestration', settings),

  // ─── Sprint 29: Agent Loop Events ─────────────────
  onAgentLoopEvent: (cb: (data: any) => void) => {
    const handler = (_event: any, data: any) => cb(data);
    ipcRenderer.on('agent:loop-event', handler);
    return () => ipcRenderer.removeListener('agent:loop-event', handler);
  },

  // ─── Sprint 32: Active Plan Update (Cline's currentFocusChainChecklist pattern) ───
  onActivePlanUpdate: (callback: (data: any) => void) => {
    const handler = (_event: any, data: any) => callback(data);
    ipcRenderer.on('chat:active-plan-update', handler);
    return () => ipcRenderer.removeListener('chat:active-plan-update', handler);
  },

  // ─── Sprint 30: Dev Console ───────────────────────
  onDevConsoleApiTraffic: (cb: (data: any) => void) => {
    const handler = (_event: any, data: any) => cb(data);
    ipcRenderer.on('devconsole:api-traffic', handler);
    return () => ipcRenderer.removeListener('devconsole:api-traffic', handler);
  },
  onDevConsoleToolRegistry: (cb: (data: any) => void) => {
    const handler = (_event: any, data: any) => cb(data);
    ipcRenderer.on('devconsole:tool-registry', handler);
    return () => ipcRenderer.removeListener('devconsole:tool-registry', handler);
  },
  onDevConsoleSettingsSnapshot: (cb: (data: any) => void) => {
    const handler = (_event: any, data: any) => cb(data);
    ipcRenderer.on('devconsole:settings-snapshot', handler);
    return () => ipcRenderer.removeListener('devconsole:settings-snapshot', handler);
  },
  getDevConsoleToolRegistry: () => ipcRenderer.invoke('devconsole:get-tool-registry'),
  getDevConsoleSettingsSnapshot: () => ipcRenderer.invoke('devconsole:get-settings-snapshot'),
  exportDevConsole: (payload: any) => ipcRenderer.invoke('devconsole:export', payload),
